...
{
    auto bytecode = m_basicReaderWriter->ReadData(filename);
...
winrt::IAsyncAction BasicLoader::LoadShaderAsync(
    std::wstring const& filename,
    _In_reads_opt_(layoutDescNumElements) D3D11_INPUT_ELEMENT_DESC layoutDesc[],
    _In_ uint32_t layoutDescNumElements,
    _Out_ ID3D11VertexShader** shader,
...
)
{
    ...
    std::shared_ptr<std::vector<D3D11_INPUT_ELEMENT_DESC>> layoutDescCopy;
    std::shared_ptr<std::vector<std::string>> layoutDescSemanticNamesCopy;
    if (layoutDesc != nullptr)
    {
        ...
        layoutDescSemanticNamesCopy.reset(
            new std::vector<std::string>(layoutDescNumElements)
        );

        for (uint32_t i = 0; i < layoutDescNumElements; i++)
        {
...
)
{
    ...
    std::shared_ptr<std::vector<D3D11_SO_DECLARATION_ENTRY>> streamOutDeclarationCopy;
    std::shared_ptr<std::vector<std::string>> streamOutDeclarationSemanticNamesCopy;
    if (streamOutDeclaration != nullptr)
    {
        streamOutDeclarationCopy.reset(
            new std::vector<D3D11_SO_DECLARATION_ENTRY>(
                streamOutDeclaration,
        ...
        streamOutDeclarationSemanticNamesCopy.reset(
            new std::vector<std::string>(numEntries)
        );
    ...
    std::shared_ptr<std::vector<uint32_t>> bufferStridesCopy;
...
{
    auto meshData = m_basicReaderWriter->ReadData(filename);
...
winrt::IAsyncAction BasicLoader::LoadMeshAsync(
    std::wstring const& filename,
    _Out_ ID3D11Buffer** vertexBuffer,
    _Out_ ID3D11Buffer** indexBuffer,
    _Out_opt_ uint32_t* vertexCount,
