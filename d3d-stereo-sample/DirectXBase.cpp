...
void DirectXBase::Initialize(winrt::CoreWindow const& window, float dpi)
{
    m_window = window;
    ...
    winrt::DisplayInformation::GetForCurrentView().StereoEnabledChanged({ this, &DirectXBase::OnStereoEnabledChanged });
}
...
void DirectXBase::CreateDeviceIndependentResources()
{
    ...
    m_dwriteFactory = dwriteFactory.as<IDWriteFactory2>();
...
void DirectXBase::CreateDeviceResources()
{
    ...
    dxgiDevice = m_d3dDevice.as<IDXGIDevice>();
...
void DirectXBase::OnStereoEnabledChanged(winrt::DisplayInformation const& sender, winrt::IInspectable const& args)
{
...
void DirectXBase::UpdateForWindowSizeChange()
{
    ...
    if (m_dpi != winrt::DisplayInformation::GetForCurrentView().LogicalDpi())
    {
        return;
    }
    ...
    else if (window.Bounds().Width != m_windowBounds.Width ||
        window.Bounds().Height != m_windowBounds.Height)
    {
        m_windowSizeChangeInProgress = true;
        CreateWindowSizeDependentResources();
    }
...
void DirectXBase::Present()
{
    ...
    if (m_windowSizeChangeInProgress)
    {
        ...
        winrt::CoreWindowResizeManager::GetForCurrentView().NotifyLayoutCompleted();
        m_windowSizeChangeInProgress = false;
    }
}
